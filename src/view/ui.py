...
class Slider:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.circle(
            surface, (255, 255, 255), (int(handle_x), int(self.y + self.height / 2)), 6
        )

        value_text = f"{self.get_value():.{self.decimals}f}"
        label_surf = self.font.render(
            f"{self.label}: {value_text}", True, (220, 220, 220)
        )
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            for i, btn in enumerate(self.buttons):
                btn_rect = pygame.Rect(
                    self.x + 60, self.y + btn["y"], self.width - 120, 44
                )
...
class FitnessGraph:
    ...
    def draw(
        self, surface: pygame.Surface, fitness_history: list[float], generation: int
    ) -> None:
        ...
        title = self.font_title.render(
            f"Фитнес (Gen {generation})", True, (220, 220, 220)
        )
        ...
        for i in range(4):
            ...
            pygame.draw.line(
                ...
                (50, 50, 65),
                (graph_x, int(y_pos)),
                (graph_x + graph_width, int(y_pos)),
                1,
            )

        # Линия фитнеса (последние 20 поколений)
        visible = (
            fitness_history[-20:] if len(fitness_history) > 20 else fitness_history
        )
        if len(visible) > 1:
            points = []
            for i, fit in enumerate(visible):
                ...
                y = (
                    graph_y
                    + graph_height
                    - ((fit - min_fit) / range_fit) * graph_height
                )
...
class GeneHistogram:
    ...
    def draw(self, surface: pygame.Surface, creatures: list) -> None:
        ...
        for i, gene in enumerate(avg_genes):
            ...
            pygame.draw.rect(
                surface, color, (x + 1, int(y), int(bar_width - 3), int(bar_height))
            )
...
class StatsScreen:
    ...
    def draw(
        ...
        subtitle = self.font_subtitle.render(
            f"Поколение: {generation}", True, (180, 180, 180)
        )
        surface.blit(
            subtitle, ((self.width - subtitle.get_width()) / 2, self.subtitle_y)
        )
    ...
    def _draw_large_graph(
        self, surface: pygame.Surface, fitness_history: list[float]
    ) -> None:
        """Большой график фитнеса."""
        if not fitness_history:
            text = self.font_text.render(
                "Нет данных (запустите симуляцию)", True, (150, 150, 150)
            )
        ...
        for i in range(5):
            ...
            pygame.draw.line(
                ...
                (50, 50, 70),
                (graph_x + 10, int(y)),
                (graph_x + graph_width - 10, int(y)),
                1,
            )

        # Линия (последние 30 поколений)
        visible = (
            fitness_history[-30:] if len(fitness_history) > 30 else fitness_history
        )
        if len(visible) > 1:
            points = []
            for i, fit in enumerate(visible):
                ...
                y = (
                    ...
                    + graph_height
                    - 10
                    - ((fit - min_fit) / range_fit) * (graph_height - 40)
                )
                points.append((int(x), int(y)))
    ...
    def _draw_large_histogram(self, surface: pygame.Surface, creatures: list) -> None:
        ...
        pygame.draw.rect(
            surface,
            (30, 30, 45),
            (hist_x, hist_y, hist_width, hist_height),
            border_radius=8,
        )
        pygame.draw.rect(
            surface,
        ...
        title = self.font_subtitle.render(
            "Средние значения генов", True, (220, 220, 220)
        )
        ...
        for i, gene in enumerate(avg_genes):
            ...
            pygame.draw.rect(
                surface, color, (x + 2, int(y), int(bar_width - 4), int(bar_height))
            )
        ...
        pygame.draw.line(
            ...
            (hist_x + hist_width - 10, int(baseline_y)),
            1,
        )
    ...
    def _draw_stats_table(
        ...
        # Заголовок таблицы
        table_title = self.font_subtitle.render(
            "Последние поколения", True, (220, 220, 220)
        )
...
class PauseOverlay:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        surface.blit(
            title, (self.x + (self.width - title.get_width()) / 2, self.y + 10)
        )
